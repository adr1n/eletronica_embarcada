...
# 225 "main.c"
    while (1)
    {
        if (EUSART_is_rx_ready()) {
            uint8_t rxChar = EUSART_Read();
            if (rxChar == 0x4C || rxChar == 0x41 || rxChar == 0x50 || rxChar == 0x52) {
                bufferRx = rxChar;
            }
            else if (rxChar == 0x0D) {
                analisaRx();
